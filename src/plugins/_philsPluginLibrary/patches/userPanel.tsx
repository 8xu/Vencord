...
function isInsideMenuContext(children: any[]): boolean {
    return children?.some(child =>
        child?.type === Menu.MenuItem ||
        child?.type === Menu.MenuGroup ||
        child?.type === Menu.MenuSeparator
    ) ?? false;
}
...
export function replacedUserPanelComponent(oldComponent: (...args: any[]) => any, thisContext: any, functionArguments: any) {
    ...
    if (!children || !Array.isArray(children)) return componentResult;

    if (isInsideMenuContext(children)) return componentResult;
